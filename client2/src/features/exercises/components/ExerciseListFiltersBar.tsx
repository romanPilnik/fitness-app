...
import type { ExerciseListSort } from '@/features/exercises/types';
import { cn } from '@/lib/utils';

const SORT_OPTIONS: { value: ExerciseListSort; label: string }[] = [
...
type Props = {
  ...
  onSortChange: (next: ExerciseListSort) => void;
  className?: string;
};

export function ExerciseListFiltersBar({
  advancedCount,
  onOpenAdvanced,
  sort,
  onSortChange,
  ...
  return (
    <section
      aria-label="Exercise list filters"
      className={cn(
        'w-full min-w-0 rounded-lg border border-(--border) bg-(--code-bg)/25 px-2 py-2 sm:px-3 sm:py-2.5',
        className,
      )}
    >
            ...
            >
              <div className="min-h-0 overflow-hidden">
                <div
                  id={sortPanelId}
                  role="radiogroup"
                  aria-label="Sort exercises in each group"
                  className={cn(

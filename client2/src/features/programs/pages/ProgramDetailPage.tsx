...
export function ProgramDetailPage() {
  ...
  if (!programId) {
    return (
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        <Link to="/programs" className="mt-4 inline-block text-sm font-medium text-(--accent)">
          Back to programs
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-6 px-4 py-8">
      ...
      <header>
        <h1 className="text-2xl font-medium text-(--text-h)">{p.name}</h1>
        {p.description ? <p className="mt-2 text-sm text-(--text)">{p.description}</p> : null}
        <p className="mt-2 text-sm capitalize text-(--text)">
          {p.status} · {p.goal} · {p.difficulty} · {p.splitType}
        </p>
        ...
        <div className="mt-4">
          {programEditing ? (
            <Button type="button" variant="secondary" onClick={() => setProgramEditing(false)}>
              Done editing
            </Button>
          ) : (
            <Button type="button" onClick={() => setProgramEditing(true)}>
              Edit program
            </Button>
          )}
        </div>
      ...
      {programEditing ? (
        <section className="rounded-xl border border-(--border) p-4">
          <h2 className="text-lg font-medium text-(--text-h)">Edit program</h2>
          <form
          ...
          <div className="mt-4 border-t border-(--border) pt-4">
            <Button
              ...
              onClick={() => {
                if (window.confirm('Delete this program permanently?')) {
                  delProgram.mutate();
                }
              }}
            >
      ...
      {programEditing ? <AddWorkoutSection programId={programId} /> : null}

      <section className="flex flex-col gap-6">
        <h2 className="text-lg font-medium text-(--text-h)">Workouts</h2>
        {workouts.map((w) => (
          <WorkoutCard
            key={`${w.id}-${programEditing ? 'e' : 'v'}`}
            programId={programId}
            workout={w}
    ...
    <div className="rounded-xl border border-(--border) bg-(--bg) p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            to={`/sessions/new?programId=${encodeURIComponent(programId)}&programWorkoutId=${encodeURIComponent(w.id)}`}
            className="inline-flex min-h-11 items-center justify-center rounded-lg border border-(--border) px-3 text-sm font-medium text-(--text-h) hover:bg-(--code-bg)"
          >
            Start workout
          </Link>
          {programEditing && !editing ? (
          ...
          {programEditing ? (
            <Button
              ...
              onClick={() => {
                if (window.confirm('Remove this workout day from the program?')) {
                  delW.mutate();
                }
              }}
            >
      ...
      {programEditing && reorder.isError ? (
      ...
      <ol className="mt-3 list-decimal space-y-3 pl-5 text-sm text-(--text)">
        {exercises.map((slot, idx) => (
          <li
            key={`${slot.id}-${programEditing ? 'e' : 'v'}`}
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          onClick={() => {
            if (window.confirm('Remove this exercise from the workout?')) del.mutate();
          }}
        >
          Remove
        </Button>
